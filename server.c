...
void validate_program_input_server(int);
...
int main(int argc, char *argv[]) {
    ...
    validate_program_input_server(argc);
...
void register_command_server(char* command) {
...
void validate_program_input_server(int argc) {

    if (argc != 1) {
        fprintf(stderr, "ERROR: Invalid input. Input has the format ./server.\n");
        exit(EXIT_FAILURE);
    }    
}